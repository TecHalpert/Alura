...
//exemplos de como exportar function


//É possível exportar apenas a função que precisa ser executada a partir de outra parte do código,
//enquanto outras funções do mesmo arquivo permanecem inacessíveis ao restante. Por exemplo:
...
//No exemplo anterior, declaramos as funções soma() e multiplica(). A função multiplica() executa soma() dentro de seu escopo e retorna um resultado.
//Podemos exportar somente a função multiplica() para ser utilizada em outras partes do código, sem a necessidade de exportar tudo.
...
//O mesmo princípio se aplica para módulos externos que instalamos em nosso projeto a partir de um gerenciador de pacotes como o NPM (com npm install <nome da lib>);
//nesse caso não precisamos passar o caminho, pois o JavaScript vai acessar os métodos necessários a partir da pasta node_modules:

const chalk = require('chalk');

// A mesma coisa para módulos que já são built-in no NodeJS. Ou seja, são módulos que integram o sistema,
// mas mesmo assim precisam ser importados para dentro do projeto, como o módulo fs (file system, ou sistema de arquivos):
...
// Quando utilizamos o ESM, o mesmo processo de exportação de módulos é feito com a sintaxe export ou export default
// e a importação com a sintaxe import < nomeModulo > from ‘./caminho/arquivo.js’.

// Esta outra forma de lidar com a importação e exportação de módulos veio com o famoso ES6 ou JS2015 e foi aos poucos sendo implementada para 
// uncionar nativamente nos navegadores com a ajuda de bundlers como o WebPack, que fazem a “tradução” de métodos do JavaScript mais moderno
// para garantir retrocompatibilidade.

// Hoje o ESM já funciona nativamente em todos os navegadores e passou a ter suporte para Node a partir da versão 13(no momento em que escrevemos este conteúdo,
// o NodeJS está na versão 16.13.2).Mesmo assim, grande parte das aplicações desenvolvidas com NodeJS ainda utiliza o 
// formato CJS de importação e exportação de módulos e as bibliotecas, pacotes e frameworks estão em processo de substituição do CJS para o ESM.
